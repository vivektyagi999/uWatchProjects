	...
	public class LocationService: Service, ILocationListener
	{
		...
		public void StartLocationUpdates()
		{
			...
			//var locationProvider = LocMgr.GetBestProvider(locationCriteria, true); 
 			...
 			if (LocMgr.IsProviderEnabled(LocationManager.NetworkProvider) || LocMgr.IsProviderEnabled(LocationManager.GpsProvider))
			{
				if (LocMgr.AllProviders.Contains(LocationManager.NetworkProvider)
						&& LocMgr.IsProviderEnabled(LocationManager.NetworkProvider))
				{
					LocMgr.RequestLocationUpdates(LocationManager.NetworkProvider, 5000, 2, this);
				}
				else
				{
					LocMgr.RequestLocationUpdates(LocationManager.GpsProvider, 1, 1, this);
			    }
			}
			else
			{
			...
			Log.Debug(logTag, string.Format("You are about to get location updates via "));
		...
		public void OnLocationChanged(Android.Locations.Location location)
		{
			...
			//IRegisterDeviceService _IRegisterDeviceService = new RegisterDeviceService();
			//var result = _IRegisterDeviceService.RegisterDevice(App.DevideId, "ANDROID", App.DeviceToken);
		}
